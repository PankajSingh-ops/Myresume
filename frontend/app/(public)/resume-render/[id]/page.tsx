...
function ResumeRenderInner() {
    ...
    useEffect(() => {
        ...
        const fetchResume = async () => {
            try {
                ...
                if (!res.ok) {
                    setError(`Failed to fetch resume (${res.status})`);
                    return;
                }
                const json = await res.json();
                setResume(json.data);
            } catch {
                setError('Network error fetching resume');
            }
        };
    ...
    useEffect(() => {
        const setReady = () => {
            const el = document.getElementById('resume-ready');
    ...
    return (
        <div id="resume-ready" style={{ width: 794, minHeight: 1123 }}>
...
export default function ResumeRenderPage() {
    return (
        <Suspense fallback={
            <div id="resume-ready" className="flex h-screen items-center justify-center bg-white">
                <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
            </div>
        }>
            <ResumeRenderInner />
